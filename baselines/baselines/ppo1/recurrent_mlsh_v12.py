...
class RecurrentMLSHV12(object):
    ...
    def policy_network(self, input, scope, size=config.baseline_layer_size,
        ...
        subpolicy_multi_cell = rnn.MultiRNNCell([self.single_cell(
            config.recurrent_hid_size, config.sub_policy_network, 'sub') for i
                                                 in range(
                config.num_sub_policy_layers)], state_is_tuple=True)

        self.sub_policies, states, alive, length = self.dynamic_rnn(
            subpolicy_multi_cell, inputs=input, batch_size=batch_size)

        self.sub_policies = layers.fully_connected(self.sub_policies,
                                                   self.action_dim)
        ...
        master_multi_cell = rnn.MultiRNNCell([self.single_cell(
            num_units=config.hid_size, cell_type=config.master_network,
            name='master') for i in range(config.num_master_layers)],
                                             state_is_tuple=True)
        ...
        self.out = layers.fully_connected(self.out, config.max_num_sub_policies)
